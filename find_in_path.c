...
/**
 * find_programme - find a program in path
...
int find_programme(data_of_program *data)
{
	...
	directories = tokenizer_path(data);/* search in the PATH */
...
/**
 * tokenizer_path - tokenize the path in directories
 * @data: a pointer to the program's data
...
char **tokenizer_path(data_of_program *data)
{
	...
	i = 0;
	tokens[i] = _str_duplicate(__strtok(PATH, ":"));
	while (tokens[i++])
	{
		tokens[i] = _str_duplicate(__strtok(NULL, ":"));
	}
