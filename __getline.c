...
/**
* __getline - read one line from the prompt.
...
int __getline(data_of_program *data)
{
		...
		(array_operators[0] == '|' && errno == 0))
	{
		...
		i = 0;
		do {
			array_commands[i] = _str_duplicate(__strtok(i ? NULL : buff, "\n;"));
			/*checks and split for && and || operators*/
			i = check_logic_opse(array_commands, i, array_operators);
...
/**
* check_logic_opse - checks and split for && and || operators
